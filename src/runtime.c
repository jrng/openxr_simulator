...
static void
xr_result_to_string(XrResult value, char buffer[XR_MAX_RESULT_STRING_SIZE])
{
...
#  define TRACE_LEAVE_RESULT(result)                            \
    do {                                                        \
        xr_result_to_string(result, trace_result_buffer);       \
        msg("leave %s -> %s\n", __func__, trace_result_buffer); \
        return result;                                          \
    } while (0)
...
static int32_t
get_string_width(Font *font, int32_t scale, const char *str)
{
    int32_t result = 0;
...
draw_string(DrawContext *ctx, Font *font, int32_t scale, int32_t x, int32_t y, const char *str, uint32_t color)
{
    ...
    while (*str)
    {
        ...
        if (glyph)
        {
            ...
            int32_t x0 = x + (scale *  glyph->x_offset);
            int32_t y0 = y - (scale * (glyph->y_offset + glyph->bound_height));
            int32_t x1 = x + (scale * (glyph->x_offset + glyph->bound_width));
            int32_t y1 = y - (scale *  glyph->y_offset);

            push_quad(ctx, (float) x0, (float) y0, (float) x1, (float) y1, u0, v0, u1, v1, color);
...
xrResultToString_impl(XrInstance instance, XrResult value, char buffer[XR_MAX_RESULT_STRING_SIZE])
{
    ...
    (void) instance;
    xr_result_to_string(value, buffer);
